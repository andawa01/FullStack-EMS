...
export const inngest = new Inngest({ id: "fullstack-emsystem" });
...
const attendanceReminderCron = inngest.createFunction(
    { id: "attendance-reminder-cron", triggers: [{ cron: "TZ=Africa/Nairobi 30 11  * * *" }] },
    // 06:00 UTC = 11:30 AM EAT
    async ({ step }) => {
        // Step 1: Get today's date range (EAT)
        const today = await step.run("get-today-date", () => {
            const startUTC = new Date(
                new Date().toLocaleDateString("en-CA", { timeZone: "Africa/Nairobi" }) + "T00:00:00+03:00"
            );
        ...
        // Step 5: Filter absent employees (not checked in and not on leave)
        const absentEmployees = activeEmployees.filter((emp) => !checkedInIds.includes(emp._id) && !onLeaveIds.includes(emp._id));
        ...
        return { totalActive: activeEmployees.length, totalAbsent: absentEmployees.length, onLeave: onLeaveIds.length, checkedIn: checkedInIds.length };
    }
